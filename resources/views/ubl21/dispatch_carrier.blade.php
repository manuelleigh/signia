        ...
        <cac:Party>
            <cac:PartyIdentification>
                <cbc:ID schemeURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06"
                        schemeAgencyName="PE:SUNAT"
                        schemeName="Documento de Identidad"
                        schemeID="6">{{ $document['subcontractor_number'] }}</cbc:ID>
            </cac:PartyIdentification>
            <cac:PartyLegalEntity>
                <cbc:RegistrationName>{{ $document['subcontractor_name'] }}</cbc:RegistrationName>
            </cac:PartyLegalEntity>
        </cac:Party>
    </cac:OriginatorCustomerParty>
    <cac:Shipment>
        ...
        <!--  INDICADORES  -->
        <!--  Indicador de retorno de transporte subcontratado  -->
        <cbc:SpecialInstructions>IndicadorTransporteSubcontratado</cbc:SpecialInstructions>
        <!--  Indicador de pagador del flete  -->
        <cbc:SpecialInstructions>Envio_IndicadorPagadorFlete_Subcontratador</cbc:SpecialInstructions>
        <cac:Consignment>
            <!--  ID OBLIGATORIO POR UBL  -->
            ...
            <cac:LogisticsOperatorParty>
                <cac:PartyIdentification>
                    <cbc:ID schemeID="6" schemeName="Documento de Identidad" schemeAgencyName="PE:SUNAT" schemeURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo06">{{ $document['subcontractor_number'] }}</cbc:ID>
                </cac:PartyIdentification>
                <cac:PartyLegalEntity>
                    <cbc:RegistrationName>{{ $document['subcontractor_name'] }}</cbc:RegistrationName>
                </cac:PartyLegalEntity>
        ...
        <cac:ShipmentStage>
            ...
            @if($document['registro_mtc'])
            <cac:CarrierParty>
                <!--  AUTORIZACIONES ESPECIALES  -->
                <cac:PartyLegalEntity>
                    <!--  NUMERO DE REGISTRO DEL MTC  -->
                    <cbc:CompanyID>{{ $document['registro_mtc'] }}</cbc:CompanyID>
                </cac:PartyLegalEntity>
            </cac:CarrierParty>
            @endif
            <!-- CONDUCTOR PRINCIPAL -->
        ...
        <cac:TransportHandlingUnit>
            ...
            <cac:TransportEquipment>
                ...
                <!--  Tarjeta Unica Circulacion / Cerificado Habilitacion Vehicular - Principal  -->
                <cac:ApplicableTransportMeans>
                    <cbc:RegistrationNationalityID>{{ $document['tuc_principal'] }}</cbc:RegistrationNationalityID>
                </cac:ApplicableTransportMeans>
                <!--  AUTORIZACIONES ESPECIALES - VEHICULO PRINCIPAL  -->
                <cac:ShipmentDocumentReference>
                    <cbc:ID schemeID="06" schemeName="Entidad Autorizadora" schemeAgencyName="PE:SUNAT" schemeURI="urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogoD37">{{ $document['registro_mtc'] }}</cbc:ID>
                </cac:ShipmentDocumentReference>
            </cac:TransportEquipment>
        </cac:TransportHandlingUnit>
    </cac:Shipment>
