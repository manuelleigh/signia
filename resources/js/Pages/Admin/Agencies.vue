                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="agency in agencies" :key="agency.id">
                                    <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{{ agency.name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">{{ agency.user?.name }} ({{ agency.user?.email }})</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                        <span class="bg-blue-100 text-blue-800 px-2 py-1 rounded-full text-xs">{{ agency.companies_count }}</span>
                                    </td>
                                ...
                                <tr v-if="agencies.length === 0">
                                    <td colspan="5" class="px-6 py-4 text-center text-gray-500">No hay agencias registradas.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <form @submit.prevent="submitBalance">
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                Recargar Saldo a: <span class="text-blue-600">{{ activeAgency?.name }}</span>
                            </h3>
