...
const applyFilters = () => {
    router.get(route('summaries.index'), filters.value, { preserveState: true, preserveScroll: true, replace: true });
};
...
const closeSummary = () => {
    ...
    setTimeout(() => { selectedSummary.value = null; }, 300);
};

const getDocumentTypeName = (code) => {
    const types = { 'RC': 'Resumen Diario', 'RA': 'Comunicación de Baja' };
    return types[code] || 'Resumen (' + code + ')';
};
...
<template>
    <Head title="Resúmenes y Bajas" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-bold text-2xl text-slate-800 tracking-tight">Resúmenes y Bajas</h2>
        </template>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mb-8">
            ...
            <div class="p-4 sm:p-6 border-b border-slate-100 bg-slate-50/50 flex flex-wrap items-end gap-4">
                ...
                <div class="w-full sm:w-auto flex-1 min-w-[150px]">
                    ...
                    <select v-model="filters.document_type" class="w-full text-sm rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500 shadow-sm">
                        ...
                        <option value="RA">Comunicación de Baja</option>
                    </select>
                </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-slate-200 text-sm bg-white">
                            ...
                            <th class="px-6 py-4 font-semibold text-slate-500 uppercase tracking-wider">Identificador</th>
                            <th class="px-6 py-4 font-semibold text-slate-500 uppercase tracking-wider">Ticket</th>
                            <th class="px-6 py-4 font-semibold text-slate-500 uppercase tracking-wider text-right">Acciones</th>
                        </tr>
                    </thead>
                    <tbody class="text-sm divide-y divide-slate-100">
                        <tr v-for="doc in summaries.data" :key="doc.id" class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-slate-600 font-mono">{{ doc.serie }}-{{ doc.number }}</td>
                            <td class="px-6 py-4 text-slate-600 font-mono">{{ doc.ticket || '-' }}</td>
                            <td class="px-6 py-4 text-right">
                                <button @click="openSummary(doc)" class="text-blue-600 hover:text-blue-800 p-2 hover:bg-blue-50 rounded-lg transition-colors">
                        ...
                        <tr v-if="summaries.data.length === 0">
                            <td colspan="7" class="py-12 text-center text-slate-500">
                                ...
                                No se encontraron resúmenes o bajas con estos filtros.
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <!-- Pagination -->
            <div v-if="summaries.links.length > 3" class="px-6 py-4 border-t border-slate-100 bg-white flex items-center justify-between">
                <span class="text-sm text-slate-500">
                    Mostrando {{ summaries.from || 0 }} a {{ summaries.to || 0 }} de {{ summaries.total }} filas
                </span>
                <div class="flex gap-1">
                    <template v-for="(link, k) in summaries.links" :key="k">
        ...
        <div v-if="showingSlideover" class="relative z-50" aria-labelledby="slide-over-title" role="dialog" aria-modal="true">
            <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity" @click="closeSummary"></div>
                            ...
                            <div class="flex h-full flex-col overflow-y-scroll bg-white">
                                ...
                                <div class="px-6 py-6 sm:px-8 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                                    <h2 class="text-xl font-bold text-slate-800" id="slide-over-title">
                                        {{ getDocumentTypeName(selectedSummary?.document_type) }}: {{ selectedSummary?.serie }}-{{ selectedSummary?.number }}
                                    </h2>
                                    <button @click="closeSummary" type="button" class="rounded-md text-slate-400 hover:text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500">
                                ...
                                <div class="flex-1 px-6 py-6 sm:px-8">
                                    ...
                                    <div v-show="activeTab === 'informacion'" class="space-y-6 text-sm text-slate-600">
                                        <div><span class="text-slate-400 block text-xs font-semibold uppercase mb-1">Fecha de envío</span> <span class="text-slate-800 font-medium">{{ formatShortDate(selectedSummary?.created_at) }}</span></div>
                                        
                                        <div><span class="text-slate-400 block text-xs font-semibold uppercase mb-1">Estado</span> 
                                            <span v-if="selectedSummary?.status === 'accepted'" class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">Aceptado</span>
                                            <span v-else-if="selectedSummary?.status === 'exception' || selectedSummary?.status === 'rejected'" class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-red-50 text-red-700 border border-red-200">Rechazado</span>
                                            <span v-else class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-amber-50 text-amber-700 border border-amber-200">En proceso (Ticket)</span>
                                        </div>

                                        <div><span class="text-slate-400 block text-xs font-semibold uppercase mb-1">Nº Ticket</span> <span class="text-slate-800 font-mono">{{ selectedSummary?.ticket || 'Sin ticket' }}</span></div>
                                        
                                        <div><span class="text-slate-400 block text-xs font-semibold uppercase mb-1">Empresa Emisora</span> <span class="text-slate-800 font-medium">{{ selectedSummary?.company?.business_name }}</span></div>
                                        ...
                                        <div class="pt-4 mt-6 border-t border-slate-100">
                                            ...
                                            <div class="flex gap-3">
                                                <button class="px-4 py-2 border border-blue-200 text-blue-600 rounded-lg hover:bg-blue-50 font-medium flex items-center gap-2 transition-colors disabled:opacity-50" :disabled="!selectedSummary?.xml_path && selectedSummary?.status !== 'accepted'">
                                                    <i class="ph ph-file-code"></i> XML
                                                </button>
                                                <button class="px-4 py-2 border border-blue-200 text-blue-600 rounded-lg hover:bg-blue-50 font-medium flex items-center gap-2 transition-colors disabled:opacity-50" :disabled="!selectedSummary?.cdr_path && selectedSummary?.status !== 'accepted'">
                                                        ...
                                                        <div class="relative flex space-x-3">
                                                            <div>
                                                                <span class="h-8 w-8 rounded-full bg-blue-500 flex items-center justify-center ring-8 ring-white">
                                                                    <i class="ph ph-arrows-clockwise text-white"></i>
                                                                </span>
                                                            </div>
                                                            <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                                                <div>
                                                                    <p class="text-sm font-semibold text-slate-800">Consulta de Ticket</p>
                                                                    <p class="mt-1 text-sm text-slate-500">El resumen con ticket {{ selectedSummary?.ticket }} ha sido consultado.</p>
                                                                </div>
                                                ...
                                                <li>
                                                    <div class="relative pb-8">
                                                        <div class="relative flex space-x-3">
                                                            ...
                                                            <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                                                <div>
                                                                    <p class="text-sm font-semibold text-slate-800">Envío de Resumen</p>
                                                                    <p class="mt-1 text-sm text-slate-500">Resumen empaquetado y enviado a SUNAT.</p>
                                                                </div>
                                                                <div class="whitespace-nowrap text-right text-xs text-slate-400">
